...
public class PostsResource extends AbstractResource {
	...
	public Response returnPostsHTML(@Context HttpHeaders headers) {
		...
		root.put("postsCount", SimpleForum.forumPosts.getNumReceivedPosts());

		if (SimpleForum.forumPosts.getNumReceivedPosts() > 0) {
			List<ForumPost> allPosts = SimpleForum.forumPosts.getAllPosts();
	...
	public Response receivePost(String newPost, @Context HttpHeaders headers) {
		...
		try {
			...
			SimpleForum.forumPosts.createNewPost(incomingForumPost);
